...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    protected class SimpleTreeNode implements BinaryTree.TreeNode<T> {
        ...
        public SimpleTreeNode(T value, SimpleTreeNode left, SimpleTreeNode right, Color color) {
            this.value = value;
            this.left = left;
            this.right = right;
            this.color = color;
        ...
        public SimpleTreeNode(T value) {
            this(value, null, null, new Color(255, 255,0));
        }
        @Override
    ...
    public int highOfTree() {
        class Inner {
           int highOfNode(SimpleTreeNode node, int level) {
               if (node == null) {
                   return 0;
               }
               int leftHeight = highOfNode(node.getLeft(), level + 1);
               int rightHeight = highOfNode(node.getRight(), level + 1);
        ...
        return new Inner().highOfNode(root, 0);
    }
    public void setColor() {
        class Inner {
            void setColor(SimpleTreeNode node, int level) {
                ...
                int green =(255 - (255 / highOfTree() * level));
                node.setColorNode(new Color(255, green, 0));
